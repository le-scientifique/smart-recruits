...
class Data:
    ...
    def __init__(self, train, test):
        train.insert(0,'ID1',train.index)
        test.insert(0,'ID1',test.index)
    ...
    def extract_ids(self):
        ...
        if "id" in first_column or "no" in first_column or "number" in first_column:
            while True:
                id_input = str(raw_input("Is %s the observation id/number?([y]/n): " % (str(self.train.columns[0]))).lower()) or "y"
                print repr(id_input)
        ...
        if id_flag == 0:
            while True:
                id_column = str(raw_input("Please enter column name of id (or type none if no such column exists): "))
    ...
    def extract_target(self):
        ...
        for colname in self.train.columns.values:
            if colname.lower() in ["response","result","target"]:
                while True:
                    target_input = str(raw_input("Is %s the target variable?([y]/n): " % (colname))) or "y"
        ...
        if target_flag == 0:
            while True:
                target_column = str(raw_input("Please enter column name of target variable (or q to quit): "))
                if target_column == "q":
                    sys.exit()
    ...
    def check_date_variables(self):
        ...
        for colname in self.train.columns:
            ...
            if "date" in colname.lower() or "do" in colname.lower():
                while True:
                    date_input = str(raw_input("Is %s a date variable?([y]/n): " % (colname))) or "y"
                ...
                while True:
                    date_conversion = str(raw_input("Choose any one: "))
    ...
    def encode_categories(self):
        ...
        if len(categorical_variables) > 0:
            ...
            while True:
                encoding = str(raw_input("Choose any one: "))
    ...
    def clean_data(self):
        ...
        self.extract_ids()
        print "extracting ids", self.train[:2]

        self.extract_target()
        print "extracting target", self.train[:2]

        #self.check_column_names()
        
        self.remove_constant_variables()
        print "removing constant variables", self.train[:2]

        self.convert_columns_to_binary()
        print "cols to binary", self.train[:2]

        self.check_date_variables()
        print "check date variables", self.train[:2]

        self.check_categorical_variables()
        print "check cats", self.train[:2]

        self.encode_categories()
        print "encode cats", self.train[:2]
